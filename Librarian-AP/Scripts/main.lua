...
local function start_gameplay_loops()
    ...
    gt_loop("book_scan", 500, function()
        ...
        -- Count checks: re-read the bookcases' slot lists a dozen cases per pass, the case the
        -- game just judged a placement on first; fires the ticks the count reaches.
        pcall(function() IA.count_pulse() end)
...
register_bp_hooks_once = function()
    ...
    -- The game's verdict on a placement. Count checks re-read that bookcase on the next pass
    -- (ItemApply.note_book_verdict); a scalar and one object read.
